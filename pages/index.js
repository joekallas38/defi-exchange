...
export default function Home() {
  ...
  // `reservedCD` keeps track of the ERC20 tokens Reserve balance in the Exchange contract
  ...
  // removeCD is the amount of `ERC20` tokens that would be sent back to the user based on a certain number of `LP` tokens
  ...
  const [tokenToBeReceivedAfterSwap, settokenToBeReceivedAfterSwap] =
    useState(zero);
  // Keeps track of whether  `Eth` or `ERC20` token is selected. If `Eth` is selected it means that the user
  // wants to swap some `Eth` for some `ERC20` tokens and vice versa if `Eth` is not selected
  ...
  const getAmounts = async () => {
    try {
      ...
      // get the amount of `ERC20` tokens held by the user
      const _cdBalance = await getCDTokensBalance(provider, address);
      // get the amount of `ERC20` LP tokens held by the user
  ...
  /**
   * swapTokens: Swaps  `swapAmountWei` of Eth/ERC20 tokens with `tokenToBeReceivedAfterSwap` amount of Eth/ERC20 tokens.
   */
  ...
  /**
   * _getAmountOfTokensReceivedFromSwap:  Returns the number of Eth/ERC20 tokens that can be received
   * when the user swaps `_swapAmountWEI` amount of Eth/ERC20 tokens.
   */
   ...
   * then we calculate the ERC20 tokens he can add, given the Eth he wants to add by keeping the ratios
   * constant
   */
  ...
  const _getTokensAfterRemove = async (_removeLPTokens) => {
    try {
      ...
      const _ethBalance = await getEtherBalance(provider, null, true);
      // get the ERC20 token reserves from the contract
      const ERC20TokenReserve = await getReserveOfCDTokens(provider);
      ...
      return (
        <div>
          <div className={styles.description}>
            ...
            {utils.formatEther(cdBalance)} ERC20 Tokens
            <br />
          ...
          <div>
            ...
            {utils.parseEther(reservedCD.toString()).eq(zero) ? (
              <div>
                ...
                <input
                  type="number"
                  placeholder="Amount of ERC20 tokens"
                  onChange={(e) =>
                    setAddCDTokens(
              ...
              <div>
                ...
                <div className={styles.inputDiv}>
                  {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
                  {`You will need ${utils.formatEther(addCDTokens)} ERC20
                  Tokens`}
                </div>
                <button className={styles.button1} onClick={_addLiquidity}>
                  Add
            ...
            <div>
              ...
              <div className={styles.inputDiv}>
                {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
                {`You will get ${utils.formatEther(removeCD)} ERC20
      ...
      return (
        <div>
          ...
          >
            ...
            <option value="ERC20Token">ERC20 Token</option>
          </select>
          <br />
          <div className={styles.inputDiv}>
            {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
    ...
    <div>
      ...
      <div className={styles.main}>
        <div>
          ...
          <div className={styles.description}>
            Exchange Ethereum &#60;&#62; ERC20 Tokens
          </div>
          <div>
            <button
        ...
        <div>
          <img className={styles.image} src="./defi.jpg" />
        </div>
      </div>

      <footer className={styles.footer}>
        Made with &#10084; by Joe Kallas
      </footer>
    </div>
  );
}